...
from codesentinelx_engine.models import Finding, SEVERITY_ORDER, ScanResult
from codesentinelx_engine.plugins import load_builtin_language_plugins
from codesentinelx_engine.scanner.controls import ExistingSecurityMeasuresAnalyzer
...
from codesentinelx_engine.scanner.file_discovery import discover_files
...
from codesentinelx_engine.scanner.role_scope import role_allows_tool, resolve_role_scope
from codesentinelx_engine.scanner.quality_benchmark import evaluate_quality_benchmark
from codesentinelx_engine.scanner.scan_cache import FileScanCache, content_sha256
from codesentinelx_engine.scanner.scan_control import honor_pause_control
from codesentinelx_engine.scanner.rules.registry import build_file_rules, build_project_rules
...
TOOL_FILE_HINTS: dict[str, dict[str, set[str]]] = {
    ...
    "findsecbugs": {"extensions": {".class", ".jar", ".war", ".ear"}, "files": {"pom.xml", "build.gradle", "build.gradle.kts"}},
    ...
    "pip-audit": {"extensions": {".txt", ".toml", ".lock"}, "files": {"requirements.txt", "pyproject.toml", "poetry.lock", "pipfile", "pipfile.lock"}},
    "safety": {"extensions": {".txt", ".toml", ".lock"}, "files": {"requirements.txt", "pyproject.toml", "poetry.lock", "pipfile", "pipfile.lock"}},
    "snyk": {"extensions": {".json", ".toml", ".lock", ".mod", ".xml"}, "files": {"package.json", "package-lock.json", "yarn.lock", "pnpm-lock.yaml", "requirements.txt", "pyproject.toml", "poetry.lock", "pipfile.lock", "go.mod", "pom.xml", "build.gradle", "gemfile", "cargo.toml"}},
