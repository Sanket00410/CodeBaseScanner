...
import type { ExportRequest, ScanRequest, ThreatModelRequest, UserRole } from "../backend/types";
...
const api = {
  ...
  createThreatModel: async (request: ThreatModelRequest) => ipcRenderer.invoke("threat:model:create", request),
  ...
  getHelpGuide: async () => ipcRenderer.invoke("help:getGuide"),
  ensureHelpHtml: async () => ipcRenderer.invoke("help:ensureHtml"),
  ensureHelpPdf: async () => ipcRenderer.invoke("help:ensurePdf"),
  getReportHistory: async () => ipcRenderer.invoke("report:history"),
  deleteReportHistory: async (payload?: { paths?: string[]; all?: boolean }) => ipcRenderer.invoke("report:delete", payload),
  getPortfolioSummary: async () => ipcRenderer.invoke("scan:portfolioSummary"),
  getScanById: async (scanId: string, role?: UserRole) => ipcRenderer.invoke("scan:getById", scanId, role),
  ...
  renderReportHtml: async (payload: { scanId: string; role?: UserRole; reportType: ExportRequest["reportType"]; reportStyle?: ExportRequest["reportStyle"]; managementContext?: ExportRequest["managementContext"] }) =>
    ipcRenderer.invoke("scan:renderHtml", payload),
  renderProfessionalHtml: async (payload: { scanId: string; projectName?: string }) =>
    ipcRenderer.invoke("scan:renderProfessionalHtml", payload),
  exportProfessional: async (payload: { scanId: string; projectName?: string }) =>
    ipcRenderer.invoke("scan:exportProfessional", payload),
