...
import { ThreatModelService } from "../backend/threatModelService";
import { ToolAccessAuthService } from "../backend/toolAccessAuth";
import { ToolManager } from "../backend/toolManager";
import { ExportRequest, ScanProgressPayload, ScanRecord, ScanRequest, ThreatModelRequest, UserRole } from "../backend/types";
...
let threatModelService: ThreatModelService | null = null;
...
async function listFilesRecursive(rootDir: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      const nested = await listFilesRecursive(fullPath).catch(() => []);
      results.push(...nested);
      continue;
    }
    if (entry.isFile()) {
...
function decodeReportRoleFromPath(relativePath: string): string {
  const firstSegment = relativePath.split(/[\\/]/).filter(Boolean)[0] || "";
  const normalized = firstSegment.replace(/[_-]+/g, " ").trim().toLowerCase();
  switch (normalized) {
    case "admin":
      return "Admin";
    case "security analyst":
    case "security analysts":
...
app.whenReady().then(async () => {
  try {
    ...
    const exportDir = path.join(app.getPath("documents"), "CodeSentinelX_Reports", "export");
    exportDirPath = exportDir;
    toolRunDirPath = path.join(app.getPath("documents"), "CodeSentinelX", "tool-runs");
    process.env.USS_EXPORT_DIR = exportDir;
...
ipcMain.handle("dialog:pickFolder", async () => {
  const result = await dialog.showOpenDialog({
    title: "Select project file or folder",
...
ipcMain.handle("threat:model:create", async (_event, request: ThreatModelRequest) => {
  if (!threatModelService) {
    throw new Error("Threat model service is not initialized.");
  }
  const projectPath = String(request?.projectPath || "").trim();
  if (!projectPath) {
    throw new Error("Project path is required to create a threat model.");
  }
  const result = await threatModelService.createThreatModel({
    projectPath,
    requestedBy: request.requestedBy || "local-user",
    framework: request.framework || "STRIDE",
  });
  const framework = result.report.framework;
  await store?.addAudit({
    action: "threat_model.created",
    actor: request.requestedBy || "local-user",
    role: "Security Analyst",
    details: `Created ${framework} threat model for ${projectPath}`,
  });
  return result;
});
...
ipcMain.handle("help:getGuide", async () => {
  if (!exportService || !scannerRootPath) {
    return { markdown: "", markdownPath: "", htmlPath: "", pdfPath: "" };
  }
  const { markdown, markdownPath } = await exportService.getUserGuideMarkdown(scannerRootPath);
  const htmlPath = await exportService.ensureUserGuideHtml(scannerRootPath);
...
ipcMain.handle("help:ensureHtml", async () => {
  if (!exportService || !scannerRootPath) {
    throw new Error("Help guide service is unavailable.");
  }
  return exportService.ensureUserGuideHtml(scannerRootPath);
});
...
ipcMain.handle("report:history", async () => {
  ...
  const files = await listFilesRecursive(exportDirPath).catch(() => []);
  const reports = files
    .filter((fullPath) => isExportReportFile(path.basename(fullPath)))
    .map((fullPath) => {
      ...
      const roleScope = decodeReportRoleFromPath(relativePath);
      return {
        fileName,
        fullPath,
        generatedAt,
...
ipcMain.handle("report:delete", async (_event, payload?: { paths?: string[]; all?: boolean }) => {
  ...
  if (requestedAll) {
    const allFiles = await listFilesRecursive(exportDirPath).catch(() => []);
    for (const filePath of allFiles) {
      if (isExportReportFile(path.basename(filePath))) {
...
ipcMain.handle("scan:getById", (_event, scanId: string, role?: UserRole) => {
  return store?.getScanView(scanId, role) || null;
});
...
ipcMain.handle("scan:export", async (_event, request: ExportRequest) => {
  ...
  if (request.format === "pdf") {
    const html = exportService.renderReportHtml(scan, request.reportType, request.reportStyle, request.role, request.managementContext);
    const destination = exportService.resolveOutputPath(scan, request.reportType, request.format, request.reportStyle, request.role);
    try {
...
ipcMain.handle(
  ...
  async (
    _event,
    payload: { scanId: string; reportType: ExportRequest["reportType"]; reportStyle?: ExportRequest["reportStyle"]; role?: ExportRequest["role"]; managementContext?: ExportRequest["managementContext"] },
  ) => {
    if (!store || !exportService) {
    ...
    return exportService.renderReportHtml(scan, payload.reportType, payload.reportStyle, payload.role, payload.managementContext);
  },
);
...
function resolveScannerRoot(appPath: string): string {
  ...
  for (const candidate of candidates) {
    const cliPath = path.join(candidate, "codesentinelx_engine", "cli.py");
